...
| 6c | **RE-SCOPED by M4, not started.** The generation loop becomes a recursion for the DIPLOID groups only: the recursion IS the grouping, so `by_parent`, `group_key`, `wanted_parents`, `kept_chains`, the per-chain veto and the contig fallback have nothing left to rediscover; `respecify` (87 lines) goes because a recursion knows the copy count before it chooses an arity; `max_generation`, `num_sites_at` and the loop scaffolding go with the schedule. The per-strand haploid pass (619 lines) STAYS -- M4 says it earns its keep, and it is why this is a restructure and not one clean recursion. `Entry::ploidy` stays (arity of the stored likelihoods) and so does **`Entry::generation`**, which an earlier draft of this row had leaving: step 9 reads it as the site's tree depth for the mosaic. Only its use as a SCHEDULING key goes -- and its third role, clamping an already-settled site to a delta (`e.generation < generation`), has to be preserved by hand as "is this the group's parent?". About -350 to -450 net, across the two largest functions on the branch (1,437 and 464 lines). | byte-identical, three contigs, plus TAP |
