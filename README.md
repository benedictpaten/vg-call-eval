...
| page | what it covers |
|---|---|
| [docs/wgs-results.md](docs/wgs-results.md) | **start here** — HG002 against T2T-Q100 v1.1 across all 24 contigs, on the 34-haplotype graph with nested calling, phasing and mosaic on. Also the same 24 contigs on ONT: ahead of short reads on SNVs and SVs, behind on indels, at 1.65x the CPU. The counts come from `scripts/wgs/bench_wgs.py`, which writes `work/<arm>/score/wgs-summary.md`; the page itself is written by hand from them |
| [docs/pangenie-comparison.md](docs/pangenie-comparison.md) | vg against PanGenie on the same graph and the same reads: vg ahead on every small-variant class, PanGenie ahead on SVs by 0.0079, on precision (vg's SV recall is the higher). vg on ONT leads PanGenie in every class, and short-read vg on SNVs and SVs. `scripts/wgs/check_comparison_page.py` asserts the page still quotes the scored arm |
| [docs/ont-hp-prior.md](docs/ont-hp-prior.md) | the fix for ONT indel accuracy falling with depth: a stronger panel prior at homopolymer-run indels (`--hp-prior`, on under `--preset ont`), fitted on chr20 and held out on chr6, +0.023 to +0.026 indel F1 at full depth with SNVs, SVs, phasing and short reads unmoved |
...
| [docs/nested-genotyping-algorithm.md](docs/nested-genotyping-algorithm.md) | **how it works now** — symbolic alleles, block decomposition, and the collect/settle/render recursion over nested chains, read out of the code and pinned to a commit |
| [docs/tier2-chr20-results.md](docs/tier2-chr20-results.md), [docs/tier2-chr6-results.md](docs/tier2-chr6-results.md) | the full six-arm accuracy tables per chromosome on the **34-haplotype** graph, small variants and SVs, and the ONT ablations (short-read defaults against `--preset ont`) on the 16-haplotype graph. Regenerated from the run artefacts by `scripts/tier2/report.py` |
...
| [docs/tier2-parameters.md](docs/tier2-parameters.md) | the caller's tuned parameters, re-swept twice: why `--mismap-max` moved to 0.95, why `--read-weight` was removed, and why `--mismap-min` stays at 0.02 even though the depth term dissolved the trade it was balancing |
