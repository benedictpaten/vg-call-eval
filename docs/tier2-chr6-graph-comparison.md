...
> **A record of the 2026-08-24 build, not the current caller.** Both columns are vg
> `v1.4.0-18654-g648296d56` (2026-08-24; CPU column added 2026-08-25), with the read arms at
> `--mismap-max 0.7` and the 10,000-edge snarl cap on every caller. Since then: the nested-calling
> fixes and read-fetch performance work, the read-walk fix (vg `28f5b88e2`, 2026-09-13), the snarl
> cap coming off under `--read-likelihood` (`f33b20367`, 2026-09-22) and `--mismap-max` 0.95
> (`0cab3fbd4`, 2026-09-23). On the current binary the `readlik` arm on the 32-haplotype hap32 graph
> (34 panel haplotypes with the CHM13 and GRCh38 paths) scores ALL GT F1 0.9774, ALL BASEPAIR F1
> 0.9425 and SV F1 0.5860, against 0.9775, 0.9090 and 0.5837 below
> (`work/tier2-chr6-hap32/results/aardvark-c6mm095`, `truvari-c6mm095s`). That run shared the
> machine with another call, so it has no cost to set beside the Cost table. That column is not
> swapped in: the 4-haplotype read and graph databases (`work/reads.gaf.db`, `work/graph.gbz.db`)
> are gone, so the other column cannot be re-run without a rebuild, and pairing a current column
> with an old one is what the one-build-per-matrix rule forbids. Current numbers: [tier2-chr6-results.md](tier2-chr6-results.md),
