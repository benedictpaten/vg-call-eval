...
while IFS=: read -r label sub contig gbzdb gafdb; do
    [ -n "$label" ] || continue
    if [ "$JOBS" -gt 1 ]; then
        # A failure inside a background job would otherwise be invisible: `wait -n` only
        # reports the job it reaped, and set -e does not cross the boundary. Log per
        # dataset and check the exit status explicitly.
        run_dataset "$label" "$sub" "$contig" "$gbzdb" "$gafdb" \
            > "$REPO/work/refresh-$label.log" 2>&1 &
        while [ "$(jobs -rp | wc -l)" -ge "$JOBS" ]; do wait -n || exit 1; done
    else
        run_dataset "$label" "$sub" "$contig" "$gbzdb" "$gafdb"
    fi
done <<< "$DATASETS"
if [ "$JOBS" -gt 1 ]; then
    while [ -n "$(jobs -rp)" ]; do wait -n || exit 1; done
