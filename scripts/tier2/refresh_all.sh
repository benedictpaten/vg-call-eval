#!/usr/bin/env bash
# Regenerate every tier-2 number from one vg build: all six arms on all four
# datasets, both benchmarks, the size-matched control, then the pages.
#
# Why all six arms and not just the ones that changed. The Poisson arms are
# untouched by a read-likelihood change, so re-running them looks like waste -- but
# the accuracy pages put all six in one table, and a table whose rows come from
...
THREADS=${THREADS:-5}
# Extra flags for the read-likelihood arms only, so a caller-side change can be
# measured across the whole matrix before it becomes a default. Empty by default.
READLIK_EXTRA=${READLIK_EXTRA:-}
# Flags that need a haplotype panel to act on, so they go to the panel-enumeration arms alone.
# --linkage-weight is refused by vg call where enumeration is not from haplotypes, so putting
# it in READLIK_EXTRA kills readlik-support.
READLIK_PANEL_EXTRA=${READLIK_PANEL_EXTRA:-}
...
if [ "${CANARY:-0}" = "1" ]; then
    if python3 "$REPO/scripts/tier2/canary.py" --vg "$VG" --threads "$THREADS"; then
        ARM_FILTER=(--only readlik readlik-nomismap readlik-nolink readlik-support)
...
run_dataset() {
    ...
    python3 "$REPO/scripts/tier2/run_arms.py" \
        ...
        ${READLIK_PANEL_EXTRA:+--readlik-panel-extra "$READLIK_PANEL_EXTRA"} \
        ${ARM_FILTER[@]+"${ARM_FILTER[@]}"}

    echo "### $label truvari"
    python3 "$REPO/scripts/tier2/truvari_sv.py" --contig "$contig" --work "$W" \
        --label "$label" --arms poisson poisson-z readlik readlik-nomismap \
        readlik-nolink readlik-support
